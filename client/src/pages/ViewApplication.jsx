...
const ViewApplication = () => {
  ...
  const fetchCompanyJobApplications = async () => {
    try {
      ...
      if (data.success && Array.isArray(data.applications)) {
        setApplicants([...data.applications].reverse()); // Correct key used
      } else {
  ...
  const changeJobApplicationStatus = async (id, status) => { // ✅ Fixed parameter name
    try {
  ...
  return applicants ? applicants.length === 0 ? (<div>No applications found.</div>) : (
            ...
            {applicants.filter(item => item.jobId && item.userId).map((applicant, index) => (
              <tr key={index} className='text-gray-700'>
                ...
                <td className='py-2 px-4 border-b text-center flex items-center'>
                  <img className='w-10 h-10 rounded-full mr-3 max-sm:hidden' src={applicant.userId.image} alt="User" />
                ...
                <td className='py-2 px-4 border-b relative'>
                  {applicant.status === "Pending" ? ( // ✅ Fixed `applicant.status`
